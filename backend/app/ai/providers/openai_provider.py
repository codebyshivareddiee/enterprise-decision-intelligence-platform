...
from app.ai.exceptions import ProviderError, InvalidResponseError, EmbeddingError, RateLimitError
...
class OpenAIProvider(LLMProvider):
    ...
    def _map_exception(self, e: Exception) -> Exception:
        ...
        if isinstance(e, openai.LengthFinishReasonError):
            return InvalidResponseError("Response was truncated due to max_tokens limit")
        return e
    ...
    async def _execute_generate(
        ...
        try:
            ...
            if completion.choices[0].message.refusal:
                raise InvalidResponseError(f"Model refused to generate output: {completion.choices[0].message.refusal}")
            ...
            if response_schema:
                try:
                    ...
                    raise InvalidResponseError(f"Failed to parse model output into schema: {e}") from e
            else:
                result = content
                
            elapsed = time.time() - start_time
            logger.info("llm_generate_success", execution_time_s=round(elapsed, 3), **log_ctx)
            return result
            
        except Exception as e:
            elapsed = time.time() - start_time
            logger.error("llm_generate_error", execution_time_s=round(elapsed, 3), error=str(e), **log_ctx)
    ...
    async def _execute_embed(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            elapsed = time.time() - start_time
            logger.info("llm_embed_success", execution_time_s=round(elapsed, 3), **log_ctx)
            return embeddings
            
        except Exception as e:
            elapsed = time.time() - start_time
            logger.error("llm_embed_error", execution_time_s=round(elapsed, 3), error=str(e), **log_ctx)
