...
class ChunkingStrategyFactory:
    ...
    def create(strategy_name: str, config: ChunkingConfig) -> DocumentChunker:
        ...
        if strategy_name == "SingleDocumentChunker":
            return SingleDocumentChunker(chunk_size=config.chunk_size, chunk_overlap=config.chunk_overlap)
        elif strategy_name == "HeadingChunker":
            return HeadingChunker(chunk_size=config.chunk_size, chunk_overlap=config.chunk_overlap)
        elif strategy_name == "SlidingWindowChunker":
            return SlidingWindowChunker(chunk_size=config.chunk_size, chunk_overlap=config.chunk_overlap)
        else:
