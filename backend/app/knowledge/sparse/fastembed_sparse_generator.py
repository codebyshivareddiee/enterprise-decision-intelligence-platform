...
class FastEmbedSparseGenerator(SparseGenerator):
    ...
    def __init__(self, model_name: str = "Qdrant/bm25") -> None:
        ...
        try:
            ...
            raise SparseGenerationError(
                f"Failed to initialize FastEmbed model {model_name}: {str(e)}"
            ) from e
    ...
    async def generate_sparse_chunks(
        ...
        try:
            ...
            raise SparseGenerationError(
                f"Failed to generate sparse vectors for chunks: {str(e)}"
            ) from e
    ...
    async def generate_sparse_query(self, query: str) -> tuple[list[int], list[float]]:
        ...
        try:
            ...
            raise SparseGenerationError(
                f"Failed to generate sparse vector for query: {str(e)}"
            ) from e
