...
class RuntimeConfig(BaseModel):
    ...
    max_retries: int = Field(default=3, description="Maximum number of retries for a step")
...
class ExecutionContext(BaseModel):
    ...
    registry: AgentRegistry = Field(description="The agent registry containing node implementations")
    planner: Planner = Field(description="The planner agent for replanning operations")
    config: RuntimeConfig = Field(default_factory=RuntimeConfig, description="Runtime configuration")
