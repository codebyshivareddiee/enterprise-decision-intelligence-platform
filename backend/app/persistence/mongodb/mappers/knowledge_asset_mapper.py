...
def to_document(asset: KnowledgeAsset) -> KnowledgeAssetDocument:
    ...
    return KnowledgeAssetDocument(
        ...
        structured_data=dict(asset.dynamic_metadata),
...
def to_domain(doc: KnowledgeAssetDocument) -> KnowledgeAsset:
    ...
    return KnowledgeAsset(
        ...
        dynamic_metadata=doc.get("structured_data", {}),
        file_path=doc.get("file_path"),
        qdrant_point_ids=doc.get("qdrant_point_ids", []),
        processing_error=doc.get("processing_error"),
        uploaded_by=UUID(doc["uploaded_by"]) if doc.get("uploaded_by") else None,
        lifecycle_state=doc.get("lifecycle_state"),
        user_description=doc.get("user_description"),
        processing_metadata=(
            ProcessingMetadata(**(doc["processing_metadata"] or {}))  # type: ignore
